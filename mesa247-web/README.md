...
Sobre cualquier pantalla flota además un **conmutador «Comensal / Anfitrión»** ([`DemoSwitch`](src/components/DemoSwitch.tsx)): las dos mitades del producto se prueban desde un mismo equipo sin escribir rutas a mano. Como el resto del andamiaje, solo aparece si la API responde a `/api/demo/locations`.
...
- **El código de país se escribe, no se supone.** El campo llega con el prefijo del local como valor inicial y editable, para que un número extranjero entre sin salir del formulario. Escribir el número entero encima de ese prefijo —con su `+`— reemplaza lo que había en vez de duplicar el país; si el código acaba repetido sin `+`, el aviso lo dice en lugar del mensaje genérico de número inválido.
